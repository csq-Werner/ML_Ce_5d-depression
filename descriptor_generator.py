...
import pandas as pd 
df = pd.read_excel(r'n_elements.xlsx')   
df.head()
df.dtypes
import numpy as np
...
composition=pd.read_excel('n_elements.xlsx',sheet_name='Sheet1', usecols="A")
composition=pd.DataFrame(composition)

predicted=np.column_stack((composition,X))
predicted=pd.DataFrame(predicted)
predicted.to_excel('to_predict_relative_permittivity_composition.xlsx', index=False,header=header)
print('The descriptors have been generated.\nPlease check the file "predict_relative_permittivity_composition.xlsx".')